#!/usr/bin/env bash
# setup-compliance-audit.sh
# DSPM_Compliance-audit-fix: 클론 → .venv 구성 → requirements 설치 → .env 준비 → API(Uvicorn) 백그라운드 실행
# 사용법:
#   bash setup-compliance-audit.sh start    # 처음 실행(또는 재실행)
#   bash setup-compliance-audit.sh stop     # 백그라운드 서버 중지
#   bash setup-compliance-audit.sh restart  # 중지 후 재실행
#   bash setup-compliance-audit.sh status   # 상태 확인
#   bash setup-compliance-audit.sh logs     # 최근 로그 보기(팔로우)
#
# 환경변수(필요 시 덮어쓰기):
#   REPO_URL="https://github.com/BOB-DSPM/DSPM_Compliance-audit-fix.git"
#   BRANCH="main"
...
#   FORCE_RESTART="1"                    # 1이면 기존 프로세스 종료 후 재시작
#   CREATE_ENV_IF_MISSING="1"            # .env 없으면 자동 생성

set -euo pipefail

### ===== 기본 설정 =====
REPO_URL="${REPO_URL:-https://github.com/BOB-DSPM/DSPM_Compliance-audit-fix.git}"
BRANCH="${BRANCH:-main}"
...
CREATE_ENV_IF_MISSING="${CREATE_ENV_IF_MISSING:-1}"

# .env 템플릿용
AWS_REGION="${AWS_REGION:-ap-northeast-2}"
MAPPING_BASE_URL="${MAPPING_BASE_URL:-http://localhost:8003}"
COLLECTOR_BASE_URL="${COLLECTOR_BASE_URL:-http://localhost:8000}"
...
MAPPING_BASE_URL=${MAPPING_BASE_URL}
...
start_api_bg() {
  ...
  ts="$(date +%Y%m%d-%H%M%S)"
  logfile="${LOG_DIR}/compliance-audit-${ts}.log"
  ...
  log "헬스체크: curl -s http://${API_HOST}:${API_PORT}/health"
  log "감사 예시: curl -s -X POST http://localhost:${API_PORT}/audit/ISMS-P/3 | jq"
  log "문서(있다면): http://localhost:${API_PORT}/docs"
}
...
logs_follow() {
  cd "$TARGET_DIR" 2>/dev/null || { warn "디렉터리 없음: $TARGET_DIR"; return 0; }
  local lastlog
  lastlog="$(ls -1t ${LOG_DIR}/compliance-audit-*.log 2>/dev/null | head -n1 || true)"
