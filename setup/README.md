...
1. Run `./setup/run-docker-stack.sh`.
   ...
   - Docker / Docker Compose가 설치되어 있지 않으면 자동으로 `get.docker.com` 스크립트를 사용해 Docker 엔진을 설치하고, `docker compose` 플러그인 또는 standalone `docker-compose` 바이너리를 내려받습니다. 설치 직후라면 현재 세션에서는 `sudo docker`가 필요할 수 있습니다.
